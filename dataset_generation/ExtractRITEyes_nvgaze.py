...
from RITEyes_helper.helperfunctions import getValidPoints, generateEmptyStorage
...
print('Extracting RITEyes: s-nvgaze')
...
PATH_DIR = os.path.join(args.path2ds, 'RITEyes', 'constraint', 's-nvgaze')
...
A = np.array([[0,0], [640, 0],[0, 480],[640, 480]])
...
for fName in list_ds[13]:
    ...
    keydict['resolution'] = []
    keydict['dataset'] = 'RITEyes-C-nvgaze'
    keydict['subset'] = str(fName)
    ...
    for ele in imList:
        ...
        maskIm = cv2.imread(path2mask, cv2.IMREAD_COLOR)[:,:,[2,1,0]]
        maskIm_woskin = cv2.imread(path2mask_woskin, cv2.IMREAD_COLOR)[:,:,[2,1,0]]
        '''
        ...
        if not np.any(maskIm_woskin):
            print('Improper mask found in package: {}, image: {}'.format(fName, ele))
            maskIm_woskin = -1*np.ones(I.shape)
        ...
        model_pupil = ransac(temp_pts, ElliFit, 8, 50, 0.1, np.round(temp_pts.shape[0]/4)).loop()
        ...
        model_iris = ransac(temp_pts, ElliFit, 15, 100, 0.01, np.round(temp_pts.shape[0]/2)).loop()
        ...
        if not noDisp:
            if fr_num == 1:
                ...
                ttl = plt.title('{}'.format(imName_withext))
                plts.add_patch(cE)
                plts.add_patch(cL)
                plt.show()
                plt.pause(.01)
                ...
                #cI.set_data(I)
                ttl.set_text('{}'.format(imName_withext))
